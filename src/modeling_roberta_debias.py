...
class RobertaForDebiasSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = RobertaTokenizer.from_pretrained('roberta-base')
        model = RobertaForDebiasSequenceClassification.from_pretrained('roberta-base')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        labels = torch.tensor([1]).unsqueeze(0)  # Batch size 1
        outputs = model(input_ids, labels=labels)
        loss, logits = outputs[:2]
...
@add_start_docstrings("""RoBERTa Model transformer with a sequence classification/regression head on top (a linear layer 
    on top of the pooled output) e.g. for GLUE tasks. """,
    ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING)
class RobertaForTransSequenceClassification(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaForTransSequenceClassification, self).__init__(config)
        self.num_labels = config.num_labels
...
@add_start_docstrings("""RoBERTa Model transformer with a sequence classification/regression head on top (a linear layer 
    on top of the pooled output) e.g. for GLUE tasks. """,
    ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING)
class RobertaForTransreSequenceClassification(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaForTransreSequenceClassification, self).__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, input_ids_t=None, attention_mask_t=None, position_ids=None, head_mask=None, inputs_embeds=None, labels=None, bias=None):
        outputs = self.roberta(input_ids,
                               attention_mask=attention_mask,
                               token_type_ids=token_type_ids,
                               position_ids=position_ids,
        ...
        if self.training:  
            outputs_t = self.roberta(input_ids_t,
            ...
            l2loss = nn.MSELoss()
            sequence_output_t = outputs_t[0]
            l2=l2loss(sequence_output, sequence_output_t)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss_fct = CrossEntropyLoss()
                if self.training:
                    loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))+ self.alpha*l2
                else:
                    loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            else:
                loss = self.debias_loss(sequence_output, logits, bias, labels)
